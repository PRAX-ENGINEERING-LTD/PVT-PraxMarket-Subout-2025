...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.viewCompany', function(event) {
            ...
            if (companyid) {
                window.open("{{ url('view-company') }}/" + companyid, '_blank');
            }
        });
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item icon-resize">
                        <a class="networkLi bs-tooltip {{ request()->is('*/dashboard*') ? 'active' : '' }}" data-bs-toggle="tooltip" title="Dashboard" data-tooltip="Dashboard View" href="{{ route('dashboard.index') }}">
                        ...
                        <a class="networkLi bs-tooltip {{ request()->is('*/network') ? 'active' : '' }}" data-bs-toggle="tooltip" title="Network" data-tooltip="Map View" href="{{ route('network.index') }}">
                        ...
                        <a class="networkLi bs-tooltip {{ request()->is('*/bookmark*') ? 'active' : '' }}" data-bs-toggle="tooltip" title="Bookmark" data-tooltip="Bookmark View" href="{{ route('bookmark.index') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" fill="none">
                        ...
                        <a class="networkLi bs-tooltip {{ request()->is('*/post*') ? 'active' : '' }}" data-bs-toggle="tooltip" title="Post View" data-tooltip="Map View" href="{{ route('post.viewBlogPages') }}">
                        ...
                        <a class="networkLi bs-tooltip {{ request()->is('*/premium-profile') ? 'active' : '' }}" data-bs-toggle="tooltip" title="Premium Profile" data-tooltip="Map View" href="{{ route('dataForPremiumProfile') }}" target="_blank">
                ...
                success: function(response) {
                    ...
                    if (response.companies && response.companies.length > 0) {
                        response.companies.forEach((company, index) => {
                            ...
                            const lng = parseFloat(company.longitude || company.lng);
                            const isValid = !isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180;
                            const imgSrc = company.cardAssets || '{{ asset("newAssets/img/dashboard/default_company.jpg") }}';
                            const img1Src = company.imgassets || '{{ asset("newAssets/img/dashboard/default_company.jpg") }}';
            ...
            success: function(response) {
                ...
                if (response.companies && response.companies.length > 0) {
                    response.companies.forEach((company, index) => {
                        ...
                        const lng = parseFloat(company.longitude || company.lng);
                        const isValid = !isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180;
                        const imgSrc = company.cardAssets || '{{ asset("newAssets/img/dashboard/12.avif") }}';
                        const img1Src = company.imgassets || '{{ asset("newAssets/img/dashboard/default_company.jpg") }}';
                        const availabilityClass = company.companyAvailablityStatus === 'IMMEDIATE' ? 'available-dashboard' :
                            company.companyAvailablityStatus === 'WEEKS' ? 'weeks-dashboard' :
                            company.companyAvailablityStatus === 'MONTHS' ? 'months-dashboard' :
...
<script>
    $(document).on('click', '.bookmark-link', function(e) {
        ...
        console.log('Making AJAX request to:', '{{ route("updateCompanyBookMarks") }}');
